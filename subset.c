...
nrrdSlice(Nrrd *nout, const Nrrd *cnin, unsigned int saxi, size_t pos) {
  ...
  if (nrrdContentSet_va(nout, func, cnin /* hide possible axinsert*/, "%d,%d", saxi,
                        pos)) {
    biffAddf(NRRD, "%s:", me);
    airMopError(mop);
    return 1;
  ...
  if (AIR_EXISTS((nin ? nin : cnin)->axis[saxi].spaceDirection[0])) {
    nrrdSpaceVecScaleAdd2(nout->spaceOrigin, 1.0, (nin ? nin : cnin)->spaceOrigin,
                          AIR_CAST(double, pos),
  ...
  */
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    ...
    if (!nrrdStateKindNoop) {
      ...
      } else if (nrrdKind2DMaskedSymMatrix == nin->axis[ai].kind && 1 == min[ai]
                 && max[ai] == szIn[ai] - 1) {
      ...
      } else if (nrrdKind3DMaskedSymMatrix == nin->axis[ai].kind && 1 == min[ai]
                 && max[ai] == szIn[ai] - 1) {
  ...
  for (ai = 0; ai < nin->dim; ai++) {
    if (AIR_EXISTS(nin->axis[ai].spaceDirection[0])) {
      nrrdSpaceVecScaleAdd2(nout->spaceOrigin, 1.0, nout->spaceOrigin,
                            AIR_CAST(double, min[ai]), nin->axis[ai].spaceDirection);
    }
  }
